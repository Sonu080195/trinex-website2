...
export default function Specialisations() {
  ...
  const closeSector = () => {
    ...
    setTimeout(() => setActiveSector(null), 350);
  };
  ...
  useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent) => { if (e.key === "Escape") closeSector(); };
  ...
  return (
    <section className="relative bg-[#F4F4F0] py-8 lg:py-10 px-4 sm:px-4 lg:px-20 overflow-hidden">
      ...
      <div
        ref={headerRef}
        className="text-center mb-6 lg:mb-10 transition-all duration-700"
      ...
      >
        ...
        <p className="text-gray-500 text-[15px] sm:text-[17px] leading-relaxed max-w-7xl mx-auto mb-4">
          Supporting contractors, developers, engineering consultancies and owners across the most
          competitive construction and infrastructure sectors.
        </p>

        <div className="flex justify-center gap-4 lg:gap-6 flex-wrap">
          {[["4", "Core Sectors"], ["20+", "Specialisations"], ["USA", "Primary Market"]].map((item, i) => (
            <div
              key={item[1]}
              className="bg-white border border-black/5 rounded-[18px] px-5 py-4 min-w-[120px] transition-all duration-500 hover:-translate-y-1 hover:border-[#C89B3C]/30"
              style={{
            ...
            >
              <h3 className="text-[#C89B3C] text-2xl font-bold">{item[0]}</h3>
      ...
      {/* MOBILE SCROLL */}
      <div ref={gridRef} className="flex md:hidden gap-4 overflow-x-auto scrollbar-hide pb-2 snap-x snap-mandatory">
        {sectors.map((sector, index) => (
          <div
            key={index}
            onClick={() => openSector(sector)}
            className="group relative min-w-[85%] h-[240px] rounded-[24px] overflow-hidden cursor-pointer snap-start shrink-0 transition-all duration-700"
            style={{
          ...
          >
            ...
            <div className="absolute bottom-6 left-5 z-10">
              <h3 className="text-white text-[30px] font-bold leading-none mb-3">{sector.title}</h3>
              <p className="text-gray-300 text-[13px] leading-6 mb-4 max-w-[240px]">{sector.description}</p>
              <div className="flex items-center gap-2">
                <p className="text-white text-[14px] font-medium">Explore Sector</p>
                <span className="text-[#C89B3C] text-lg transition-transform duration-300 group-hover:translate-x-1">→</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="hidden md:grid grid-cols-2 lg:grid-cols-4 gap-8">
        {sectors.map((sector, index) => (
          <div
            key={index}
            onClick={() => openSector(sector)}
            className="group relative h-[340px] rounded-[28px] overflow-hidden cursor-pointer border border-black/5 shadow-[0_4px_20px_rgba(0,0,0,0.04)] hover:-translate-y-2 hover:shadow-[0_20px_60px_rgba(0,0,0,0.12)] transition-all duration-500"
            style={{
              opacity: gridInView ? 1 : 0,
              transform: gridInView ? "translateY(0)" : "translateY(28px)",
              transition: `opacity 0.6s ease ${index * 120}ms, transform 0.6s ease ${index * 120}ms, box-shadow 0.5s, border-color 0.5s`,
            }}
          >
            <div
              className="absolute inset-0 bg-cover bg-center transition duration-700 group-hover:scale-110"
            ...
            <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-[#C89B3C] to-[#E8B84B] scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />

            <div className="absolute bottom-8 left-7 z-10">
              <h3 className="text-white text-[40px] font-bold leading-none mb-4">{sector.title}</h3>
              <p className="text-gray-300 text-[15px] leading-7 mb-5 max-w-[280px]">{sector.description}</p>
              ...
              <div className="flex items-center gap-3">
                <p className="text-white text-lg font-medium">Explore Sector</p>
                <span className="text-[#C89B3C] text-2xl transition-transform duration-300 group-hover:translate-x-2">→</span>
              </div>
            </div>
          </div>
        ))}
      ...
      {activeSector && (
        <div
          className="fixed inset-0 z-[999] bg-[#07111F]/96 backdrop-blur-xl overflow-y-auto transition-opacity duration-300"
          style={{ opacity: popupVisible ? 1 : 0 }}
        >
          {/* Background grid, matches site sections */}
          <div
          ...
          <div className="relative max-w-7xl mx-auto px-5 sm:px-8 py-14 sm:py-20">
            ...
            {/* TOP */}
            <div
              className="flex items-start justify-between mb-6 sm:mb-16 gap-5 transition-all duration-500"
              style={{
            ...
            >
              <div>
                <div className="inline-flex items-center gap-3 mb-4 sm:mb-5">
                  <span className="h-px w-8 bg-[#C89B3C]" />
                  <p className="text-[#C89B3C] uppercase tracking-[3px] sm:tracking-[4px] text-[11px] sm:text-sm">
              ...
              <button
                onClick={closeSector}
                aria-label="Close"
                className="w-12 h-12 sm:w-14 sm:h-14 rounded-full border border-white/10 flex items-center justify-center text-white hover:bg-white/10 hover:border-[#C89B3C]/40 hover:text-[#C89B3C] active:scale-95 transition-all duration-300 shrink-0"
              >
                <X size={24} />
              </button>
            </div>
            ...
            <div className="flex lg:hidden gap-4 overflow-x-auto scrollbar-hide pb-2 snap-x snap-mandatory">
              {activeSector.subsectors.map((item, index) => (
                <Link
                  href={item.link}
                  key={index}
                  className="group relative min-w-[88%] h-[240px] rounded-[24px] overflow-hidden block cursor-pointer snap-start shrink-0 transition-all duration-500"
                  style={{
                    opacity: popupVisible ? 1 : 0,
                    transform: popupVisible ? "translateY(0)" : "translateY(20px)",
                ...
                >
                  ...
                  <div className="absolute bottom-6 left-5 z-10">
                    <h3 className="text-white text-[24px] font-bold leading-tight mb-3 max-w-[220px]">{item.title}</h3>
                    <div className="flex items-center gap-2">
                      <p className="text-white text-[14px] font-medium">Explore</p>
                      <span className="text-[#C89B3C] text-lg transition duration-300 group-hover:translate-x-2">→</span>
                    </div>
                  </div>
                </Link>
              ))}
            ...
            <div className="hidden lg:grid grid-cols-3 gap-8">
              {activeSector.subsectors.map((item, index) => (
                <Link
                  href={item.link}
                  key={index}
                  className="group relative h-[300px] rounded-[28px] overflow-hidden block cursor-pointer transition-all duration-500"
                  style={{
                    opacity: popupVisible ? 1 : 0,
                    transform: popupVisible ? "translateY(0)" : "translateY(24px)",
                    transitionDelay: `${100 + index * 80}ms`,
                  }}
                >
                  <div
                  ...
                  <div className="absolute bottom-8 left-7 z-10">
                    <h3 className="text-white text-3xl font-bold leading-tight mb-4 max-w-[260px]">{item.title}</h3>
                    <div className="flex items-center gap-3">
                      <p className="text-white text-lg font-medium">Explore</p>
                      <span className="text-[#C89B3C] text-2xl transition duration-300 group-hover:translate-x-2">→</span>
                    </div>
                  </div>
                </Link>
              ))}
