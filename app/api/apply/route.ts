...
export async function POST(req: NextRequest) {
  try {
    ...
    const type =
      (formData.get("type") as string) || "job_application";
    ...
    const email =
      ...
      (formData.get("Email Address") as string) ||
      "";
    ...
    const jobTitle =
      (formData.get("job_title") as string) || null;

    const firstName =
      (formData.get("first_name") as string) || null;
    ...
    const desiredRole =
      (formData.get("desired_role") as string) || null;
    ...
    const serviceNeeded =
      (formData.get("service_needed") as string) || null;

    const hiringFor =
      (formData.get("hiring_for") as string) || null;

    const volume =
      (formData.get("volume") as string) || null;
    ...
    if (file && file.size > 0) {
      ...
      // Signed URL for immediate email notification.
      // Valid for 30 days.
      // Long-term access should use resume_path instead.
      ...
      // Permanent storage path used to regenerate
      // signed URLs later.
    ...
    // ─────────────────────────────────────────────
    // Determine email notification recipient
    // ─────────────────────────────────────────────
          ...
          ? `
            <p>
              ...
              <small>
                ...
                For permanent access, use the admin panel.
              </small>
            </p>
          `
          : ""
    ...
    await resend.emails.send({
      from:
        "RUDRON Website <careers@rudrongts.com>",
